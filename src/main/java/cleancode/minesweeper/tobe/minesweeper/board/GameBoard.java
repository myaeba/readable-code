...
//        openSurroundedCells(cellPosition);
    ...
    private List<CellPosition> calculateSurroundedPositions(CellPosition cellPosition, int rowSize, int colSize) {
        return RelativePosition.SURROUNDED_POSITIONS.stream()
                ...
                .map(cellPosition::calculateSurroundedPositions)
...
//        Deque<CellPosition> deque = new ArrayDeque<>();
    ...
    private void openAndPushCellAt(Stack<CellPosition> stack) {
        CellPosition curruntCellPosition = stack.pop();
        if (isOpenedCell(curruntCellPosition)) {
            return;
        }
        if (isLandMineCellAt(curruntCellPosition)) {
            return;
        }

        openOneCellAt(curruntCellPosition);

        if (doesCellHaveLandMineCount(curruntCellPosition)) {
            return;
        }

        List<CellPosition> surroundedPositions = calculateSurroundedPositions(curruntCellPosition, getRowSize(), getColSize());
